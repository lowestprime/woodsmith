    ...
    <HeaderShell>
      <Shell className="header-inner">
        ...
        <nav aria-label="Primary" className="site-nav">
          {site.navigation.filter((item) => String(item.href) !== "/search").map((item, index) => <Link className="nav-link-pill" href={item.href} key={item.href} {...inlineEditAttrs({ resource: "settings", field: "navigation", index })}>{item.label}</Link>)}

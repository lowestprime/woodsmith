...
## Conditional website-generated BCC (B2)

`conditional-notification-routing.ts` defines bounded, deterministic rules over persisted B1 intent/topic/source/channel and canonical piece identity/availability. `queueNotificationEmail` resolves the legitimate stored inquiry ID for `customer_inquiry_admin` and `commission_submitted`; planner confirmation also verifies the Project relation. The ordinary recipient resolver unions conditional copies with global/type/event BCC and excludes To/CC. Account links bypass these rules entirely; provenance/fail-closed retry is unchanged. Quarantine cannot queue matching mail, and existing delivery snapshots are immutable under routing edits.

Authenticated Notifications Overview integrates `conditional-routing-editor.tsx` with typed versioned autosave, operation replay protection, redacted audit, explicit clear and stale-version recovery. Its saved-rule preview is hypothetical and never persists or sends examples. The independently versioned `notification-conditional-routing` settings entry defaults to empty without a write; no schema migration or owner-setting/template replacement occurs. SMTP identity, credentials, provider forwarding, later project correspondence and production deployment are separate. See [routing semantics](docs/notification-routing.md) and [B2 acceptance](docs/b2-routing-audit-20260907.md).
...
## Goal F branch architecture — release pending

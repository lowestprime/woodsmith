...
## Goal F branch workflows — pending release
...
Goal-F branch Process editing includes **Preview before publishing** for existing/new administrator notes and seller notes. It renders current fields privately, with refresh/close controls, without changing publication. This is locally validated and awaits the final release; see [family evidence](docs/goal-f-family-reconciliation.md).

Goal-F account mutation hardening (release pending): account rename/delete and all dependent references now share one transaction, including the legacy save path. The last administrator cannot be demoted or removed. Deletion revokes sessions and removes account carts rather than turning them into guest carts. Fault-injection rollback, reopen and role protections pass; see [family evidence](docs/goal-f-family-reconciliation.md).

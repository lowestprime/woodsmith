...
test("application and audit images expose the exact source revision at build and runtime", () => {
  ...
  assert.match(runnerStage, /ENV WOODSMITH_BUILD_SHA=\$\{WOODSMITH_BUILD_SHA\}/);
  ...
  assert.match(audit, /ENV WOODSMITH_BUILD_SHA=\$\{WOODSMITH_BUILD_SHA\}/);
});

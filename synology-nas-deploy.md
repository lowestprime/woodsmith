...
In the deployed application, media synchronization inspects primary JPEG marker structure and accepts complete images with long appended payloads. It records unavailable status for empty, unreadable, invalid-signature, and detectably malformed/truncated images without altering originals. The earlier final-4-KiB end-marker heuristic falsely rejected valid source photographs and is superseded. Public piece galleries continue to omit known-unavailable images; editorial review is independent.
...
- Goal E proved SMTP acceptance/auth-mail isolation, and Goal F preserved that configuration. If transport later fails, the account UI reports the actual failure.
...
## Historical Goal-E immutable runtime control
...
## Current Goal-F immutable runtime and migrations

Goal F uses Compose project `woodsmith-f-release` on external network `woodsmith_default`. Its protected `releases/goal-f-20260922/accepted-compose.yml` and `immutable-runtime-override.json` select the exact image recorded above through the existing root environment mechanism. Do not rebuild or run the historical Goal-E Compose commands for routine Goal-F operation. The stopped `woodsmith-goal-e-retained-goal-f` is intentional rollback retention; `woodsmith-v19-retained-goal-e` remains deliberate historical retention. The completed production transaction and fresh recovery manifest are indexed in [Goal-F release evidence](docs/goal-f-release-evidence-20260922.md).

The deployed Goal-F application advances SQLite through migrations 17–19 for commerce reconciliation, conservative public-credit correction and business ownership. Multi-worker mode remains disabled after migration. The complete transaction has passed. Keep the Goal-E image, retained rollback container, fresh paired recovery and staged-restore evidence. Actual Goal-E compatibility with schema 19 and return to Goal F were verified. An older image must not be pointed casually at migrated data; use the proven paired recovery/rollback procedure.
...
The estimator/capacity/category packet adds no schema, environment variable or dependency. It is deployed in the exact Goal-F image after verified recovery/staged-restore/recreation/rollback/return. See [packet evidence](docs/goal-f-family-reconciliation.md).

The Goal-F account correction also requires no migration/configuration change. In the deployed application, customer sessions with unverified email cannot claim private records; password changes invalidate existing sessions. Profile-image delivery and pending/verified isolation retain their candidate and final-source acceptance.

The private Process preview uses existing Markdown/media dependencies and adds no environment variable or schema change. Include admin/new/seller draft preview and unchanged public publication state in final candidate acceptance.

...
Legitimate general/piece inquiries queue only the `customer_inquiry_admin` operator notice; legitimate planner submissions retain their customer confirmation and Project workflow. Quarantine queues neither. The primary correspondence address remains the configured Builder email, seeded as `woodsmithbb@proton.me`. New-install forwarding defaults are empty. Existing saved forwarding preferences, category policies, templates and account-link recipient protections remain intact. B2 can add private conditional BCC to these operator notices and planner confirmations by resolving the persisted legitimate inquiry ID. Rules use normalized intent/topic/surface/route/channel and canonical piece identity/availability at submission, never arbitrary hidden labels, message keywords, uploads or tokens. See [notification routing](notification-routing.md) for management, preview and exact copy semantics.

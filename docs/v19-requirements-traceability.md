...
- Application checks after the Settings/Categories autosave slice: 165/165 tests, TypeScript, lint with zero errors and 11 established raw-image advisories, and the safe Next.js 16.3.0 standalone build passed.
...
| R7 | Every ordinary Studio edit preserves viewport/focus without navigation | PARTIAL / LOCAL GREEN FOR THIRTEEN EDITOR GROUPS | The shared queue/navigation shell now covers Page, Piece, Project, Process, People, Custom Type, Order, Review, Settings/site structure, Categories, notification policy/template, and visitor-policy editors. Disposable authenticated browser acceptance proved same-document, URL, focus, and field-position continuity for the seven newly migrated groups. Media metadata/source-folder-rule fields remain submit-based. | Migrate the remaining Media ordinary editors, then prove exact scroll, focus, selection, history and failure behavior across the complete desktop/mobile matrix. |
| R8 | Autosave all non-destructive Studio edits | PARTIAL / LOCAL GREEN FOR THIRTEEN EDITOR GROUPS | `767b84c` provides debounce, immediate controls, blur flush, serialization/coalescing, optimistic concurrency, bounded retry, local failure retention, canonical adoption and accessible status. Process, People, Custom Type, Order, Review, Settings/site structure, and Categories now use typed canonical actions with durable replay binding, redacted audit records, targeted invalidation, manual fallback saves, and confirmed deletion where applicable. Adoption is not yet complete for Media metadata/source-folder rules, so Save buttons remain. | Complete the remaining Media panel row, add conflict/offline/multi-item browser evidence, and remove Save buttons only after field-complete parity. |
...
Save controls remain as manual queue flush/fallback while parity is incomplete. The next implementation slice must migrate Media metadata/source-folder-rule fields through coordinated typed mutation queues and add complete browser evidence before removing those controls.

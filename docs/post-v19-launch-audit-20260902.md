...
## Goal D: public copy and profile proportions (2026-09-08)

Goal D completes the remaining public-copy/profile work on the existing launch branch. About's 1440px baseline portrait was 1372×1715px and its card about 1861px tall; the final avatar is 112×112px and the card about 187px. Public forms are bounded at 48rem, reading content at 72ch and public shells at 96rem; About is narrower and Studio retains its full-width workspace. Uploaded/generated avatars share 28px compact, 80px editor and responsive 80/112px public variants. Local blob previews bypass media URL resolution and are released after use.

Copy changes remove customer-visible backend/mail terminology, two exact legacy media-workflow detail arrays and duplicate availability/footer labels. The v2 data-only refinement uses the existing history table and a one-time settings marker, with transaction/rollback/retry/idempotence and byte-exact custom-content protection. Schema and A/B1/B2/C semantics remain unchanged.

Acceptance:
- Chromium: 15 representative routes × 320/390/430/768/1024/1440/1920 × light/dark = 210 passing cases. Firefox: the same 15 architectures × 320/390/1440 × light/dark = 90 passing cases.
- Direct routes: About, home, Portfolio, inventory piece detail, Shop, populated cart, Contact, planner, Process listing/article, care document, search results, authenticated profile/projects and authorized customer request. These cover each changed shell, grid, detail, form, reading and avatar architecture. Thirty supplementary rendered checks cover the other seeded pieces/posts, journal redirects, authentication and validation/empty/missing states; shared layouts use the directly tested representatives.
- Zero overflow, broken fixture images, framework overlays, WCAG A/AA violations, unexpected page/console errors or unexplained cross-origin requests in the matrix. Ten cross-browser interaction groups cover progress/soft navigation/history/dynamic height/reduced motion, contact context/focus/fail-closed configuration, lightbox focus/zoom/Escape, planner navigation and avatar preview/initials.
- Six uploaded-photo/long-owner-profile cases preserve content and geometry at 320/390/1440 in both browsers. Sixteen 200% reflow cases cover About, Portfolio, Contact and account profile in both themes: Chromium native tab zoom at 2× (1424 to 712 CSS pixels); Firefox browser pixel scale 2× (683 CSS pixels). No overflow or accessibility violations.
- Final focused tests 40/40; full application suite 254/254; typecheck PASS; lint zero errors/six established image warnings; production build and standalone runtime-data gate PASS. The final blob-preview fix was covered by rebuilt-runtime interactions and a focused regression. The unchanged layout matrix remains applicable.

Evidence was produced from the unreleased branch in a disposable standalone runtime with synthetic media/accounts and no production mutation. The profile fixture was restored. Provider-live delivery/payment, production media identity and final production-clone/recovery/deployment evidence remain Goal E; the existing Node SQLite experimental warning remains. Screenshots, runtime databases and temporary scripts stay outside Git. Goal D introduces no visual-audit contract changes, so its build was not rerun.

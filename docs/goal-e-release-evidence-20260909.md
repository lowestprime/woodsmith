...
| Candidate/recovery/live/Git | Exact clean replacement candidate and post-Turnstile paired backup/staged restore PASS (identities below). NAS candidate smoke PASS. Pending: immutable deploy, live providers/intake/routing/auth, persistence, rollback/return, PR/master integration and branch consolidation. |
...
Resume the EXISTING Goal: minimally recheck current clean Git and candidate/runtime/environment identities. Revalidate or refresh paired recovery if production/environment/media changed after this checkpoint. NAS exact-candidate smoke is now complete; prepare a fail-closed production/rollback transaction, then deploy the recorded image without rebuilding. Finish all live/persistence/rollback/master-only gates before marking complete. Do not use the rejected dc322fab image or its failed recovery attempt.


## NAS smoke and resumed safe checkpoint

- Resumed from clean checkpoint `dcb9a20f1f7ffb91d71fd3b9cb0421d91a03ccef`, equal to upstream and remote. Application/Docker/Compose inputs still match accepted `a019ec6c`; no rebuild.
- NAS candidate image configuration, revision and all RootFS diff IDs match the accepted manifest. Transport SHA-256 and final recovery manifest SHA-256 match the recorded values.
- Current production database and accepted backup compare equal across every table's schema and sorted row contents; SQLite quick_check is ok. Current media names, sizes and modification times match all 3,187 manifest entries. Current runtime environment, backup environment and staged environment match. Managed Turnstile fields are nonempty; values were not exposed. Secret-bearing environment and resolved Compose files remain restricted outside Git.
- Exact NAS image passes 11 representative public, commerce, commission, search, account and Studio-login HTTP routes with UID/GID 1026:100 and resolved accepted Compose environment. The disposable container has no network access or published ports, a copy of the backup database, restored real media mounted read-only, and a separate writable cache. It reaches schema 16, passes quick_check, reads both recovery helpers, writes/removes scoped data/cache probes, and serves an actual 4,013,377-byte JPEG with the expected signature. Production media write/persistence and provider-live behavior remain production gates.
- Supplemental rollback preparation: accepted v19 starts against a copy of the upgraded schema-16 database and passes six public/account/Studio-login routes plus quick_check. Comparison identifies only media scan timestamps/preview-status/reason metadata and derived search-index changes; other tables remain equal. This is **not** the required production rollback/return proof. Choose and verify the final rollback state procedure before production mutation.
- Restricted results: `releases/goal-e-20260909/resume-revalidation.json`, `nas-smoke-result.json`, and `nas-smoke/compatibility.json`. The initial compatibility comparison correctly reports differences and is retained for diagnosis; do not treat it as blanket byte-equality.
- Both disposable NAS smoke containers were removed. Production is still container `651cebbd441245c374045b14d3392d826aa3dccf1cf7957cf18509ef5262d02a`, image `904bf2785c37…`, started `2026-08-31T16:48:53.622800709Z`; internal and HTTPS return 200.

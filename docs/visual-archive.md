...
## Final v19 archive result

The released application is `0067488abb058829f3b94584c02ea666e552c9a8`. Audit-only repairs through `686a69c0cc5011394f35add750c29663626990f8` leave the application `site` tree byte-identical and are bound separately in the Tier-3 evidence.

- Exact Tier 1 snapshot-lab `local-lab-smoke-20260831T035148Z-0067488a-749c5c277c` passed.
- Exact Tier 1 full read-only `local-readonly-full-20260831T035502Z-0067488a-b198068c12` passed.
- Production-clone Tier 2 `tier2-local-full-20260831T042104Z-0067488` passed 1,863 route records, 23,493 observations, 5,961 captures, restart-idempotent schema migration, exact clone-write accounting, media/placeholder checks, checksums, and cleanup.
- Live-production smoke `tier3-live-smoke-20260901T042509Z-0067488-686a69c-6ef843ed` passed.
- Full live-production `tier3-live-full-20260901T042651Z-0067488-686a69c-e79d0ed1` passed 1,784 routes, 22,347 observations, 5,948 captures, 5,337 checksums, task uniqueness, expected-blocked Cloudflare Insights provenance, zero unapproved cross-origin traffic, zero successful unsafe requests, and cleanup.

The final Tier-3 acceptance record is `/home/cbeaman/woodsmith-release-evidence/tier3/tier3-live-full-20260901T042651Z-0067488-686a69c-e79d0ed1.acceptance.txt`. Its manifest, validation, and checksum-set SHA-256 values are recorded in [`v19-release-evidence-ledger-20260901.md`](v19-release-evidence-ledger-20260901.md). Earlier failed archives remain superseded diagnostic evidence and must not be substituted for this run.
...
Source- and database-inventoried canonical routes use the complete matrix above. Rendered same-origin links discovered from those pages use desktop, tablet, and mobile representatives in both themes plus archival desktop dark. The protected inventory also declares stable, query-addressable Studio views for the search index, Projects editor, top-level Visitors, and Notifications overview, types, templates, delivery, audit, and SMTP workspaces. This accounts for every finite link target without repeating deep captures for query variants that render the same page template. `coverage-plan.json` records both matrices and the sampling rationale.

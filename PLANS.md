...
| WP08 release evidence and deployment | ACTIVE | `3057090` adds protected inventory schema 3, stable v19 Studio view routes, deep audit/dialog captures, inventory-derived clone-only autosave/search mutation evidence, exact write-count validation, and narrow completed-response abort classification. Content-equivalent local smokes passed: snapshot-lab 445 captures/48 routes/six states/ten successful clone writes and live-readonly 366 captures/36 routes/zero successful writes, both with zero unexpected diagnostics and complete cleanup. R1-R13 traceability and the Studio form classification are in `docs/v19-requirements-traceability.md`. Existing Process, People, Custom Type, Order, Review, Settings, coordinated home/footer structure, and Categories records now use typed durable autosave. Disposable authenticated browser acceptance proved same-document URL/focus/field-position continuity, reload persistence, serial collection writes, Cancel-first category deletion with Escape focus restoration, SQLite integrity, and one operation/audit row per tested save. The post-hardening source passes 165/165 application tests, TypeScript, lint with zero errors and 11 established advisories, the safe standalone build, and 84/84 visual-audit tests. Media metadata/source-folder rules are the final ordinary-editor parity row. Exact candidate images/Tier 1, production-clone migration/performance/Tier 2, paired recovery, deployment, Tier 3, persistence, and rollback remain active. |
...
| Remaining product work | PENDING | Complete coordinated R7/R8 autosave and destructive-dialog parity for media metadata/source-folder rules; then freeze exact images and run final Tier 1, production-clone migration/performance/Tier 2, paired recovery, candidate deployment, Tier 3, persistence/rollback, sidecar reachability, Cloudflare differential, PR reconciliation, and post-deployment verification. |

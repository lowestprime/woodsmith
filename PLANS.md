...
Goal-F follow-up: the two dining-table identities are now exact-byte/visual/live proven; all 23 published pieces have usable photography. See [media truth](docs/goal-f-media-truth-20260919.md). The exact absent Goal-E probe index row awaits guarded cleanup only after fresh release recovery. Local sidecar startup was missing and its installed package stale; the owner-logon supervised task, UNC runner fix and package alignment now pass authenticated production health, two-image inference/cache reuse, CUDA, supervised restart and semantic/offline fallback. See [sidecar proof](docs/goal-f-local-sidecar-20260919.md). This adds one PowerShell runner correction after checkpoint f2bbf259; final Goal-F source gates/release remain pending. Production application, schema and data are unchanged.

Goal-F reconciliation follow-up: the existing 316-row matrix now records 224 VERIFIED_COMPLETE, 4 SUPERSEDED, 15 PROVIDER_BLOCKED, 2 OPTIONAL_PROVIDER_ENHANCEMENT and 71 ACTIVE_GAP clauses. These are intermediate counts, not final completion. [Current security/UX proof](docs/goal-f-current-security-ux-20260919.md) confirms redirects/host-only HSTS, authenticated cookie behavior, foreign-origin rejection, theme SSR/persistence and bounded reflow, and explicitly records absent CSP. Remaining source/live/release obligations stay active. No Goal-F production application transition has started.
...
Final local gates now pass at application tree tested from `0e193e2`: 310 application tests, 144 visual-audit tests, typecheck/lint/build/visual build and zero production audit vulnerabilities. See [exact results](docs/goal-f-final-local-gates.md). Candidate and all production/recovery/browser/Git closure obligations remain open.
...
- DONE: authenticated top-level `/studio?panel=visitors`, Visitors title/active navigation, legacy child-link redirect, URL-preserved range/session page, and save-queue protection. Notifications has six views, retains Audit and independent optional `visitor_session` type/delivery semantics, and no longer loads/renders visitor analytics.
- DONE: retained local map library with fixed 960×720 viewBox, reserved 4:3 geometry and bounded parent sizing; inert graphical map with semantic country/region totals; proportionate daily trend with UTC numeric visitor/session/pageview values. Current-day pageviews are now included in the rolling-window trend so its total matches the KPI. Missing/unresolved geography remains unassigned. Ten-session pagination, full path/referrer wrapping, compact mobile KPIs and confirmed retention controls preserve the existing privacy/data model.
- PASS: 29/29 focused visitor/privacy/navigation/chart/UI checks. The two repaired test files pass 10/10; final full `npm run test` passes 244/244. The earlier three failures were obsolete schema-15 assertions and a pre-B1 contact-form source assertion; only tests were repaired, preserving B1/B2 implementation.
- PASS: `npm run typecheck`, `npm run lint` (zero errors, eight established image warnings), `npm run build` including standalone runtime-data gate, and `npm --prefix visual-audit run build`. Node's existing experimental SQLite warning remains.
- PASS: direct Chromium and Firefox checks at 320/390/430/768/1024/1440/1920, each light/dark (28 cases): geometry, no horizontal overflow, country/daily equivalents, numeric reconciliation and zero accessibility violations. Ten interaction/authentication cases cover canonical navigation, history/reload, Notifications separation/type/Audit, keyboard and purge cancellation/failure recovery. Actual native 200% browser zoom passes both browsers and themes (1440 physical → 720 CSS pixels). Zero unexpected page/console errors or cross-origin traffic; one console error was deliberately induced for network-failure recovery.
- DONE: operator, architecture and audit-inventory documentation aligned; stale seven-tab acceptance expectations updated to six. No schema migration, external map/analytics service, production data mutation, deployment, or A/B reconstruction. Final artifacts remain outside Git; only disposable Goal C runtime/browser resources are removed.

Next manual Goal: **D public copy/profile/high-yield UX**. Goal E release/recovery/deployment/consolidation remains later manual work. Stop after this packet.
...
**Goal A outcome: DONE.** The coherent technical slice is the commit containing this closure entry; its full identity and upstream/cleanup verification are retained in `media-goal-a/closure.json` under the restricted post-v19 run root. Evidence: 32 focused checks, 67 related-media checks, typecheck/lint/build, four hash-verified real JPEG strict decodes, production-clone preservation/rollback/idempotence/reopen, and 20 Chromium/Firefox rendered media checks across initial/reload/reindex/restart runs. No application source changed after the final passing build/browser checks. See [the Goal A audit](docs/post-v19-launch-audit-20260902.md#goal-a-technical-media-recovery-2026-09-05).

The two easel JPEGs previously called truncated are complete and strictly decodable; the old trailer heuristic was wrong. Two dining-room-table indexed paths remain absent with unproven same-folder rename candidates; do not reassign them without identity provenance. Original files and editorial identities are preserved. Production deployment and later roadmap gates remain pending. Stop after this packet's push and scoped runtime cleanup.

### Later Goals — start manually, do not implement in Goal A

1. **Customer intake/routing:** retain `woodsmithbb@proton.me` as primary; operator-private BCC rules for `wbeaman1@gmail.com` and `cooperbeaman@gmail.com`; structured intent/type/keyword routing and auth-mail isolation. Direct inbound Proton forwarding is provider-side only. Add server-validated Turnstile and deterministic SEO/marketing spam quarantine before project creation; exclude spam from Projects and lead-time counts.
2. **Visitors:** top-level `/studio?panel=visitors`, removed from Notifications; correct map/trend scaling and aspect with accessible text equivalents. Validate 320/390/430/tablet/small-laptop/1440+ in both themes and Chromium/Firefox.
3. **Public copy/profile/high-yield UX.**
4. **Final QA/storage/recovery/deployment/master-only consolidation.** The full roadmap below remains durable; Goal A completion does not claim these later gates.
...
| Public UX and copy | COMMITTED; TARGETED RENDERED QA PASS | `4f2d33e` preserves schema-v14/public-launch work; `f9d0409` repairs missing-media fallbacks and upward-scroll mobile navigation. 112 public and 80 authenticated route/layout/theme checks plus eight real-photo/lightbox checks pass. Production-clone fidelity remains required. |
| Global progress and accessibility | TARGETED RENDERED QA PASS | CSS timeline and passive/rAF fallback pass top/middle/bottom, dynamic-height and navigation-reset checks; Chromium 149 and Firefox 151 feature-enabled timeline verified. Full release accessibility/performance acceptance remains required. |
| Studio modernization | ROUTING + DENSE MEDIA ACCEPTANCE PASS; REMAINING AUDIT OPEN | Routing acceptance remains green. A 150-record disposable media library passes in-place pagination/autosave/rename/delete, keyboard grid navigation, crop/assignment, reload persistence, mobile pane focus, and 1440/430/390/320 light/dark reflow. Private inspector images now bypass the unauthenticated optimizer. Other dense Studio panels and final release acceptance remain open. |
...
| Production-clone migration | PASS FOR CURRENT SCHEMA | Fresh read-only schema-13 snapshot passes v14/v15, customization preservation, failure rollback/retry, repeat migrations and two application initializations; source bytes unchanged. Exact hashes in the launch audit. Not a paired recovery backup or media-rendering proof. |
...
- Draft child PR: `#7`
- Current work packet: `WP08` closure
...
| WP06 privacy-preserving visitor insights and audit log | DONE / LOCAL VALIDATION COMPLETE | Additive schema version 12 replaces new raw visitor identifiers with purpose-separated keyed pseudonyms, scrubs legacy sensitive fields, records minimized pageviews, and adds policy-controlled retention. Studio includes a top-level responsive Visitors workspace and Notifications Audit with unique/session/pageview metrics, an accessible world map/list, pagination, cohort labels, bounded purge, filtered redacted audit detail, and redacted JSON export. Full application tests pass 142/142; typecheck, lint, safe Next.js 16.3.0 production build, dependency audit, all Compose configurations, 80/80 visual-audit tests, `linux/amd64` image filesystem inspection, disposable container/browser acceptance, schema/retention/redaction checks, and desktop/mobile dark/light rendering pass. No production data or media was mounted or mutated; NAS release gates remain WP08 work. |
| WP07 FTS5 lexical-first search | DONE / LOCAL VALIDATION COMPLETE | Additive schema version 13 installs a managed Unicode61/prefix FTS5 index with source-write synchronization, publication/private visibility, deletion/rename handling, weighted BM25 snippets, audited integrity/rebuild controls, and immediate lexical streaming. Optional semantic work reads only precomputed vectors for the first 24 lexical candidates, is capped at 2.5 seconds, and cannot replace the lexical fallback. Focused search/migration tests pass 16/16 and the full application suite passes 153/153; typecheck, lint, safe production build, high-threshold dependency audit, all Compose configurations, 80/80 visual-audit tests, `linux/amd64` image/filesystem inspection, and disposable authenticated browser acceptance pass. A 5,000-document warm lexical test enforces p95 <=250 ms; the disposable browser measured first-useful p95 152.1 ms with zero console/page errors, no overflow, no reload/scroll drift, correct weighted ranking, single-character Unicode input, and public/admin visibility. This is local synthetic evidence, not the required exact production-clone/LAN budget proof. No production data or media was mounted or mutated. |
| WP08 release evidence and deployment | DONE / DEPLOYED / FINAL TIER 3 PASS | Deployed application `0067488abb058829f3b94584c02ea666e552c9a8` passed exact snapshot-lab, full Tier 1, production-clone Tier 2, paired backup/staged restore, immutable deployment, post-deploy route/database/search/SMTP/sidecar checks, forced-recreation persistence, rollback/return-to-candidate, and public ingress checks. Audit-only repairs through `686a69c0cc5011394f35add750c29663626990f8` leave the `site` tree unchanged and passed the final Tier-3 smoke plus full live-production archive. The full run `tier3-live-full-20260901T042651Z-0067488-686a69c-e79d0ed1` passed capture, diff, report, validation, uniqueness, security, checksums, and cleanup across 1,784 routes, 22,347 observations, and 5,948 captures. Exact paths, hashes, image IDs, diagnostic history, and residual caveats are in `docs/v19-release-evidence-ledger-20260901.md`. |

Final release note: runtime evidence is immutable and complete. PR #7 and PR #6 must now be integrated in order without squash, rebase, amend, or force-push. The final restricted handoff records the resulting merge commits and evidence tag.

Superseded predeployment diagnostic note: exact Tier 2 run `lab-20260814T071432Z-05428be8` was stopped and retained after strict unsafe-method accounting identified ten unexpected clone-only `POST /studio` requests. Media-picker browse, pagination, and search had been implemented as Next server actions even though they are read-only. The repair moves those operations to an authenticated, private, no-store `GET /api/studio/media-library` boundary while leaving media selection on the existing transactional autosave path. A disposable rendered-browser regression observed exactly two successful picker GETs, no picker-originated Studio POST, no console warning/error, and an unchanged deterministic fingerprint across all 47 SQLite tables. The failed archive remains superseded diagnostic evidence; its required replacement tiers are the passing final runs indexed above.

Superseded Tier 2 diagnostic note (2026-08-29): the preserved exact run `tier2-local-full-20260828T141236Z-3663066b` completed naturally against the verified production clone. Capture and reporting completed with 83,005 captures across 3,871 routes and 125,880 checksums; the twelve successful unsafe requests were the expected isolated snapshot-lab writes, cross-origin requests remained zero, source hashes remained stable, SQLite migrated idempotently with `quick_check=ok`, and cleanup removed every run resource except the retained output volume. That run attributed validation failure to two allegedly truncated JPEGs in the chair-mounted adjustable artist easel folder. Goal A now supersedes that diagnosis: both exact source files have complete primary JPEG codestreams, valid strict decoding, and long trailing payloads. Their repeated Studio observations produced 124 failed visible-media checks and four diagnostics. The repair performs bounded source-signature checks during media synchronization, records preview status without overwriting editorial metadata, excludes known-unavailable images from public display, and renders explicit audited fallbacks across Studio pickers, inspectors, crop tools, relation editors, and project media. Original bytes remain unchanged. The failed archive is retained as diagnostic evidence only and was superseded by the passing exact final release tiers indexed above.

## 2026-07-11 Sitewide UX, Data, Commission, And Visual Archive Overhaul

- Status: RELEASE GATES COMPLETE AND DEPLOYED; PR #6 INTEGRATION READY
...
| Deterministic visual archive | DONE / EXACT TIER 1, TIER 2, AND TIER 3 PASS | The QA slice provides protected bounded inventory, strict live-readonly and isolated snapshot-lab modes, deterministic high-resolution capture, deep UI/dialog states, searchable HTML/PDF reports, checksums, diagnostics, diffs, locks, and retention. Final Tier-3 evidence uses distinct deployed-app `0067488` and audit-runner `686a69c` provenance, contains no duplicate logical/capture identities, records the exact blocked Cloudflare Insights infrastructure traffic, rejects every unapproved cross-origin request, and passed the full live-production validator. |
| Three-tier media and placeholder evidence | DONE / EXACT RELEASE EVIDENCE PASS | Exact Tier 1 synthetic, Tier 2 production-clone, and Tier 3 live-production gates all passed mounted-media, placeholder, provenance, diagnostics, and checksum validation. Earlier `3663066b` and other failed runs remain retained diagnostic evidence only. |
...
| Sidecar queue, supervision, and GPU arbitration | DONE / LOCAL CUDA AND PRODUCTION REACHABILITY VERIFIED | Added strict CPU/CUDA selection, explicit SentenceTransformer device binding, bounded batch/allocator memory, single-flight work, cross-process lease ownership, indexed-cache queue status, crash supervision, authenticated health/wrong-token probing, and resumable cache semantics. A twelve-image representative benchmark measured CPU median 0.488176s versus CUDA 0.129774s (about 3.76x), identical eight-label rankings, maximum score drift 0.000105856, and 670 MiB peak reserved VRAM. Post-deployment checks proved authenticated health, wrong-token rejection, and final logs. The visual archive remains CPU/SwiftShader, preventing current GPU contention. |
...
| Remaining product work | RELEASE GATES COMPLETE / PR CLOSURE ONLY | Exact images, Tier 1, production-clone Tier 2, paired recovery, deployment, Tier 3, persistence, rollback, sidecar reachability, Cloudflare differential, and post-deployment verification are complete. Only ordered PR integration, evidence tagging, and the final restricted handoff remain after this documentation commit. |
...
- **Lexical-first search** (`search-index.ts`, `search-rerank.ts`, `search.ts`): additive schema v13 maintains an FTS5 index in source-write transactions; BM25/prefix/snippet results render first; optional semantic work reads only precomputed vectors for the first 24 lexical candidates and leaves lexical order intact when disabled, offline, or timed out.
...
| 43 | DONE / OPTIONAL PROVIDER DEPENDENCY | FTS5 BM25 lexical search, Unicode-safe prefixes/snippets, same-transaction visibility/deletion synchronization, client-side reference-image tag analysis, an audited Studio integrity/rebuild workspace, and bounded precomputed-vector reranking are implemented. Provider-backed semantic enrichment remains optional behind `ENABLE_EMBEDDING_SEARCH`; disabled/offline/timeouts preserve the immediate lexical result. |

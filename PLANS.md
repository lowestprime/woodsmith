...
| WP08 release evidence and deployment | ACTIVE | `3057090` adds protected inventory schema 3, stable v19 Studio view routes, deep audit/dialog captures, inventory-derived clone-only autosave/search mutation evidence, exact write-count validation, and narrow completed-response abort classification. Content-equivalent local smokes passed: snapshot-lab 445 captures/48 routes/six states/ten successful clone writes and live-readonly 366 captures/36 routes/zero successful writes, both with zero unexpected diagnostics and complete cleanup. R1-R13 traceability and the Studio form classification are in `docs/v19-requirements-traceability.md`. All existing ordinary Studio record fields, including Media metadata and source-folder rules, now use typed durable autosave; consequential create, apply, upload, rename, batch, model, notification, fulfillment, and destructive operations remain explicit. Rebuilt disposable browser acceptance proved same-document URL/focus/field-position continuity, pending-save flush on media selection, serial collection writes, Cancel-first deletion with Escape focus restoration, SQLite integrity, strictly increasing canonical versions, and exactly one operation/audit row per tested autosave. The current repair source passes 187/187 application tests, TypeScript, lint with zero errors and eight established advisories, the safe standalone build, and 95/95 visual-audit tests. Exact candidate images/Tier 1, repaired production-clone Tier 2, deployment, Tier 3, persistence, and rollback remain active. |

Predeployment evidence note: exact Tier 2 run `lab-20260814T071432Z-05428be8` was stopped and retained after strict unsafe-method accounting identified ten unexpected clone-only `POST /studio` requests. Media-picker browse, pagination, and search had been implemented as Next server actions even though they are read-only. The repair moves those operations to an authenticated, private, no-store `GET /api/studio/media-library` boundary while leaving media selection on the existing transactional autosave path. A disposable rendered-browser regression observed exactly two successful picker GETs, no picker-originated Studio POST, no console warning/error, and an unchanged deterministic fingerprint across all 47 SQLite tables. The exact images and all release tiers must be regenerated from the repair commit; the failed archive remains superseded diagnostic evidence, not release evidence.

Tier 2 diagnostic note (2026-08-29): the preserved exact run `tier2-local-full-20260828T141236Z-3663066b` completed naturally against the verified production clone. Capture and reporting completed with 83,005 captures across 3,871 routes and 125,880 checksums; the twelve successful unsafe requests were the expected isolated snapshot-lab writes, cross-origin requests remained zero, source hashes remained stable, SQLite migrated idempotently with `quick_check=ok`, and cleanup removed every run resource except the retained output volume. Validation failed because two source JPEGs in the chair-mounted adjustable artist easel folder are truncated: each has a valid JPEG start but no end-of-image marker. Their repeated Studio observations produced 124 failed visible-media checks and four diagnostics. The repair performs bounded source-signature checks during media synchronization, records preview status without overwriting editorial metadata, excludes known-unavailable images from public display, and renders explicit audited fallbacks across Studio pickers, inspectors, crop tools, relation editors, and project media. Original bytes remain unchanged. The failed archive is retained as diagnostic evidence only; source changed afterward, so exact images and all release tiers must be regenerated from the repair commit.
...
| Deterministic visual archive | IMPLEMENTED / CURRENT SOURCE GATES GREEN; FINAL EXACT TIERS PENDING | The QA slice provides protected bounded inventory, strict live-readonly and isolated snapshot-lab modes, deterministic tiled high-resolution capture, deep UI/dialog states, searchable HTML/PDF reports, checksums, diagnostics, diffs, locks, and retention. Inventory schema 3 covers the v19 search, Projects, and Notifications subviews and derives six/seven mutation states plus ten/twelve successful clone-write expectations. Snapshot-lab now drains independent read-only routes through a bounded laptop pool before entering a one-worker mutation phase and FIFO write/rollback handler, preventing transient clone state from leaking into parallel screenshots. All 95 visual-audit tests pass. Release evidence is computed on Laptop Docker Desktop; the NAS remains the production, snapshot-export, immutable-storage, and rollback control plane. Earlier exact evidence remains an immutable regression baseline only; code changed afterward, so final exact Tier 1, Tier 2 production-clone, candidate deployment, Tier 3 live-production, and rollback evidence must be regenerated at one final identity. |
| Three-tier media and placeholder evidence | IMPLEMENTED / CURRENT LOCAL TIER 1 GREEN; FINAL EXACT TIER 1/2/3 PENDING | Schema-v5 archives identify Tier 1 synthetic, Tier 2 production clone, or Tier 3 live production. Protected inventory returns only public-media counts and SHA-256 fingerprints; browser evidence hashes direct and Next-optimized mounted sources. Deterministic reports reject provenance mismatch, missing mounted files, synthetic production markers, absent anonymous mounted-media observations, load failures, and unapproved visible placeholders. The current 95-test archive suite passes. The preserved `3663066b` Tier 2 attempt correctly failed on two truncated production-source JPEGs; the subsequent fallback repair requires a fresh exact Tier 1, production-clone Tier 2, and NAS Tier 3. |
...
| Remaining product work | PENDING | Freeze exact images and run final Tier 1, production-clone migration/performance/Tier 2, paired recovery, candidate deployment, Tier 3, persistence/rollback, sidecar reachability, Cloudflare differential, PR reconciliation, and post-deployment verification. |
